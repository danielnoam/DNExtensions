...
namespace DNExtensions.Systems.ControllerRumble
{
    ...
    /// </summary>
    [AddComponentMenu("DNExtensions/Controller Rumble/Controller Rumble Source")]
    public class ControllerRumbleSource : MonoBehaviour
    {
        ...
        [SerializeField, ShowIf("is3DSource"), Min(0f)] private float minRumbleDistance = 1f;
        [Tooltip("Distance at which rumble completely stops (further = no rumble)")]
        [SerializeField, ShowIf("is3DSource"), Min(0f)] private float maxRumbleDistance = 10f;
        [Tooltip("Falloff curve from full intensity (at 0) to no rumble (at 1)")]
        [SerializeField, ShowIf("is3DSource")] private AnimationCurve distanceFalloffCurve = AnimationCurve.EaseInOut(0, 1, 1, 0);
        ...
        public float MinDistance => minRumbleDistance;
        public float MaxDistance => maxRumbleDistance;
        public AnimationCurve DistanceFalloffCurve => distanceFalloffCurve;
        
        
        private void OnValidate()
        {
            if (minRumbleDistance >= maxRumbleDistance)
            {
                maxRumbleDistance = minRumbleDistance + 1f;
            }
        }
